...
def get_dict_cpg_gene_island(type):
...
def get_dict_cpg_gene_island_shore(type):
    ...
    for i in range(0, len(cpg)):
        ...
        if len(curr_cpg) > 2:
            if curr_cpg[0:2] == 'cg':
                if curr_cpg_type == 'Island' or curr_cpg_type == 'N_Shore' or curr_cpg_type == 'S_Shore':
                    if curr_chr != 'X' and curr_chr != 'Y':
                        if len(curr_gene) > 0:
                            all_genes = list(set(curr_gene.split(';')))
                            dict_cpg_gene[curr_cpg] = all_genes
