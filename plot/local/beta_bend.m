...
fn = sprintf('../../data/%s/result/gene/approach/bend/linreg/%s/any/%s/%s/bend_%d.txt', ...
    base, ...
    gender, ...
    data_type, ...
    geo, ...
...
genes = bend_data.textdata(1:num_genes);
...
for gene_id = 1:size(genes, 1)
    gene_name = string(genes(gene_id));
    ...
    slope_less = bend_data.data(gene_id, 2);
    intercept_less = bend_data.data(gene_id, 3);
    ...
    slope_more = bend_data.data(gene_id, 7);
    intercept_more = bend_data.data(gene_id, 8);
    ...
    hold all;
    h = plot(ages_less, gene_data_less, 'o');
    ...
    hold all;
    h = plot(ages_more, gene_data_more, 'x', 'Color', color);
    set(get(get(h, 'Annotation'), 'LegendInformation'), 'IconDisplayStyle', 'off');
