...
for cpg_id = 1:size(cpgs, 1)
    ...
    if strcmp(config.gender, 'versus')
        config.gender = 'F';
        config.color = 'r';
        plot_linreg_variance_ols_window_cpg(config, cpg)
        config.gender = 'M';
        config.color = 'b';
        plot_linreg_variance_ols_window_cpg(config, cpg)
        config.gender = 'versus';
    ...
    savefig(f, sprintf('%s/%d_linreg_variance_window_%s.fig', save_path, cpg_id, suffix))
    saveas(f, sprintf('%s/%d_linreg_variance_window_%s.png', save_path, cpg_id, suffix))
end
